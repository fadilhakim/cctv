...
class delete extends CI_Controller {
	...
	public function delete_blog() {
		...
		$getdata['getimage'] = $this->model_blog->getblogById($id);
		...
		if(!empty($row))
		{
		  ...
		  $image1 = $row->blog_image;
		  $image2 = $row->blog_thumbnail;

		  if(!empty($image1) && file_exists(FCPATH . "assets/image/blog/".$image1))
		  {
			  unlink(FCPATH . "assets/image/blog/".$image1);
		  }

		  if(!empty($image2) && file_exists(FCPATH . "assets/image/blog/".$image2))
		  {
			  unlink(FCPATH . "assets/image/blog/".$image2);

		  }

		  $this->model_delete->delete_blog($id);
